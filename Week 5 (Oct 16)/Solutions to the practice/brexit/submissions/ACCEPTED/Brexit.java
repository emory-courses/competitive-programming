...
public class Brexit {
    ...
    public boolean magic(graph g, boolean verbose) {
        ...
        queuemap.put(l, g.vertexmap.get(l));

        while (queue.size() > 0) {
            vertex v = (vertex)queue.remove();
            queuemap.remove(v.id);
            visited.put(v.id, v);
            if (verbose) out("Pop " + v.id);
            for (edge e : v.edges) {
                if (e.v1.id == v.id && !visited.containsKey(e.v2.id) && !queuemap.containsKey(e.v2.id)) {
                    --e.v2.current;
                    if (verbose) out("Leave? " + e.v2.id + ": " + e.v2.leave() + ", want " + e.v2.original + "/" + e.v2.current);
                    if (e.v2.leave()) {
                        queue.add(e.v2);
                ...
                else if (e.v2.id == v.id && !visited.containsKey(e.v1.id) && !queuemap.containsKey(e.v1.id)) {
                    --e.v1.current;
                    if (verbose) out("Leave? " + e.v1.id + ": " + e.v1.leave() + ", want " + e.v1.original + "/" + e.v1.current);
                    if (e.v1.leave()) {
                        queue.add(e.v1);
        ...
        return g.vertexmap.get(x).leave();
    }
    ...
    public class vertex {
        ...
        public boolean leave() {
            return this.current <= this.original / 2; 
        }
