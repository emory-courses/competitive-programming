...
public class Brexit_wrong {
	static BufferedReader in = new BufferedReader(new InputStreamReader(System.in));

	public Brexit_wrong() {}

	public static void main(String[] args) throws Exception {
		new Brexit_wrong().doProblem();
	}
    ...
    public boolean magic(graph g, boolean verbose) {
        ...
        while (queue.size() > 0) {
            ...
            ArrayList<edge> edges = (ArrayList<edge>)v.edges.clone();
            for (edge e : edges) {
                e.v1.edges.remove(e);
                e.v2.edges.remove(e);
                ...
                if (e.v1.leave() && !visited.containsKey(e.v1.id) && !queue.contains(e.v1)) queue.add(e.v1);
                if (e.v2.leave() && !visited.containsKey(e.v2.id) &&  !queue.contains(e.v2)) queue.add(e.v2);
            }
        }
    ...
    public class vertex {
        ...
        public boolean leave() {
            // THIS IS WRONG!! (Wrong calculation of half of trading partners leaving)
            if (original % 2 == 0)
                return this.edges.size() <= original / 2;
            return this.edges.size() <= (original + 1) / 2;
        }
